...
module.exports = {
    ...
    register: async (req, res, next) => {
        try {
            ...
            const user = new User({ role: 'admin', ...result})
    ...
    login: async (req, res, next) => {
        try {
            /*  only user with role admin should be able to login*/

            const result = await authLoginSchema.validateAsync(req.body)
            const user = await User.findOne({ email: result.email }) 

            if(!user) throw createError.NotFound('User not registered')
            if(user.role !== "admin") throw createError.BadRequest('Access Denied, please login as admin.')
